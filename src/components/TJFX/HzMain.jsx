...
import styles from './HzMain.less';
import DealChartData from './DealChartData';
...
export default class TJFX extends Component {
  ...
  render() {
    const { dwmc, dispatch, treeList, list, ColumnsData, loading, type} = this.props;
    ...
    const TableList = {
      ...
      scroll: INTEGRATE?
        ColumnsData.scroll?{x: ColumnsData.scroll, y: h - 260}: {y: h - 260}:
        ColumnsData.scroll?{x: ColumnsData.scroll, y: fullScreen ?  h - 288 : h - 374}: {y: fullScreen ? h - 288 : h - 374},
    };
    const ChartList={
      type: type,
      list,
            ...
            <div style={{width: '100%', padding: '10px'}}>
              <Row className={styles.title}>
                ...
                {/*<Col span={4} style={{textAlign: 'right'}}>*/}
                  ...
                  {/*}}><Icon type="bars"/></Button>*/}
                  {/*<Button onClick={() => {*/}
                    {/*this.setState({showCharts: true})*/}
                  {/*}}><Icon type="pie-chart"/></Button>*/}
                {/*</Col>*/}
              </Row>
              <Search {...SearchList} />
              {
                showCharts ? <DealChartData {...ChartList} /> : <Table {...TableList} />
              }
            </div>
          </TreeLayout>
        </div>
