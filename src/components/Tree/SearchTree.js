...
import {Tree, Input, Popover, Icon} from 'antd';
...
class SearchTree extends PureComponent {
  ...
  render() {
    const {showSearch, treeData, searchValue, expandedKeys, autoExpandParent} = this.state;
    const loop = data => data.map((item) => {
      ...
      if (item.children) {
        return (
          <TreeNode key={`node-${item.id}`} title={<span style={{fontWeight: 'bold'}}><Icon
            className={classnames(styles.Icon, styles.Folder)}
            type={folderOpen ? "folder-open" : "folder"}/>{title}</span>} data={item}>
            {loop(item.children)}
          </TreeNode>
        );
      }
      ...
      if (popContent) {
        return <TreeNode key={`node-${item.id}`}
                         title={<Popover placement="right" content={popContent} title="详细信息"><Icon
                           className={classnames(styles.Icon, styles.File)} type="file-jpg"/>{title}</Popover>}
                         data={item}/>;
      }
      return <TreeNode key={`node-${item.id}`}
                       title={<span><Icon className={classnames(styles.Icon, styles.File)}
                                          type="file-jpg"/>{title}</span>} data={item}/>;
    });
