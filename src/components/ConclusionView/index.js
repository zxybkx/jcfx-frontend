...
class JLZSForm extends Component {
  ...
  onClick = () => {
    ...
    validateFields((errors) => {
      ...
      dispatch({
        type: `${basePath}/saveConclusion`,
        payload: {
          bmsah: ajxx.bmsah,
          tysah: ajxx.tysah,
          ysay: ajxx.ysay_aymc,
          bigResult: config.djl,
          smallResult: config.xjl,
          id: config.id,
          note: data.note,
        }
      });
  ...
  render() {
    const {fact, ajxx, dispatch, stage, allProblem} = this.props;
    const basePath = stage === 'ZJ' ? 'zcjd': 'gsjd';

    let option = {};
    switch (ajxx.ysay_aymc) {
      case '交通肇事罪':
        option = require(`../../data/${basePath}/ss_option.json`);
        break;
      case '危险驾驶罪':
        option = require(`../../data/${basePath}/wxjsz_ss_option.json`);
        break;
      case '走私、贩卖、运输、制造毒品罪':
        option = require(`../../data/${basePath}/zspmyszzdpz_ss_option.json`);
        break;
      default:
        option = require(`../../data/${basePath}/ss_option.json`);
        break;
