...
import ReloadButton from './Controls/Reload';
import ScaleButton from './Controls/Scale'
...
class ReactEditor extends React.Component {
  ...
  getCustomToolbar = () => {
    const toolbarCustomButtons = [
      ...
      <ReloadButton editorState={this.state.editorState} onClick={this.reloadData} config={{}}/>,
    ];
  ...
  render() {
    ...
    return (
      <div className={classnames(styles.ReactEditor, fullScreen ? styles.fullScreen : '', initFullScreen ? styles.initFullScreen : '')}>
        <Spin style={{width: '100%', position: 'absolute', textAlign: 'center', marginTop: '60px'}} spinning={this.state.loading} tip="正在加载文档..." size="large"/>
        {
          !this.state.loading &&
