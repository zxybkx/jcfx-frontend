...
class SketchField extends PureComponent {
  ...
  _initContextMenu = () =>{
    ...
    let canvas = this._fc;
    //bind contextmenu event for user interface
    $(`#${this._canvas.id}`).next(".upper-canvas").unbind('contextmenu').bind('contextmenu', function(ev) {
      let pointer = canvas.getPointer(ev.originalEvent);
      let objects = canvas.getObjects();

      let hasSelected = false;
      for (let i = objects.length - 1; i >= 0; i--) {
        let object = objects[i];
        if (object.type !== 'image' && object.containsPoint(pointer)) {
    ...
    zoom = (factor) => {
      ...
      let newCanvasWidth = canvas.getWidth() / preFactor * factor;
      let newCanvasHeight = canvas.getHeight() / preFactor * factor;
      this._container.style.width = `${newCanvasWidth + 2}px`;
      this._container.style.height = `${newCanvasHeight + 2}px`;
      ...
      for (let i in objects) {
        ...
        if(obj.type === 'image'){
          ...
          obj.setCoords();
          this._rate = obj.scaleX;
        }else {
          obj.scaleX = obj.scaleX / preFactor * factor;
          obj.scaleY = obj.scaleY / preFactor * factor;
          obj.left = obj.left / preFactor * factor;
          obj.top = obj.top / preFactor * factor;
          obj.setCoords();
        }
      }
...
//===========**********================
  ...
  //add by saber
  removeActiveObject = () =>{
    this._fc.remove(this._fc.getActiveObject());
  };
