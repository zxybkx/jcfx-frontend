...
export default class GlobalHeader extends PureComponent {
  ...
  render() {
    const {
      ...
      onNoticeVisibleChange, onMenuClick, onNoticeClear, onLogout, onFeedback,
    } = this.props;
    const menu = (
            ...
            <Dropdown overlay={menu}>
              <span className={`${styles.action} ${styles.account}`}>
                <Avatar className={styles.avatar} style={{backgroundColor: '#1890ff', verticalAlign: 'middle'}}>
                  {currentUser.name.length > 2 ? currentUser.name.substr(2) : currentUser.name.substr(1)}</Avatar>
