...
export default class Window extends Component {
  ...
  constructor(props) {
    ...
    this.windowWidth = props.container ? $(props.container).width() : $(window).width();
    this.windowHeight = props.container ? $(props.container).height() :$(window).height();
    this.restoreCoords = {
      x: this.lastX,
      y: this.lastY,
      w: this.lastWidth,
  ...
  componentDidMount() {
    ...
    if (commonWindows) {
      //如果指定了x， y 直接使用
      const {clientWidth: cw, clientHeight: ch} = this.props.container ? this.props.container  : document.body;
      const {w, h} = this.state;
      const x = !_.isNil(this.props.x) && this.props.x >= 0 ? this.props.x : (cw - w) / 2;
      const y = !_.isNil(this.props.y) && this.props.y >= 0 ? this.props.y : (ch - h) / 2;
      this.setState({
        x: cw / 4,
        y: 0,
      }, ()=> {
        this.setRestoreCoords();
        this.props.onResize && this.props.onResize(this.state.w, this.state.h);
      });
  ...
  resize = (w, h, x, y) => {
    ...
    this.setState({
      ...
      this.setRestoreCoords();
      this.setBodySelection(true);
      const {clientWidth: w, clientHeight: h} = this.window;
      this.props.onResize && this.props.onResize(w, h);
    });
  };
  ...
  render() {
    const {title = '数据编辑', icon = 'edit', fixed = true, closable = true , exportFile} = this.props;
